...
instance Read PasswdEntry where
    readsPrec _ value =
        case split ':' value of
             ...
             x ->
               error $ "Invalid number of fields in input: " ++ show x
        where
...
type UIDMap =
  Map.Map Integer PasswdEntry


type UserMap =
  Map.Map String PasswdEntry
...
inputToMaps inp =
  ...
  where
    ...
    uidmap =
      Map.fromList . map (uid &&& id) $ entries

    usermap =
      Map.fromList . map (userName &&& id) $ entries
...
main = do
  args <- getArgs
  when (length args /= 1) $ do putStrLn "Syntax: passwdmap filename"
                               exitFailure
...
mainMenu :: (Show a1, Show a, Read k, Ord k) => (Map.Map k a, Map.Map String a1) -> IO ()
mainMenu maps@(uidmap, usermap) = do
  ...
  case sel of
    "1" ->
      lookupUserName >> mainMenu maps

    "2" ->
      lookupUID >> mainMenu maps

    "3" ->
      displayFile >> mainMenu maps

    "4" ->
      return ()

    _ ->
      putStrLn "Invalid selection" >> mainMenu maps
  where
    lookupUserName = do
      putStrLn "Username: "
      username <- getLine
